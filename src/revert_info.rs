...
impl RevertInfo {
    ...
    pub fn new(
        ...
        if let Ok(error_signal) = ErrorSignal::try_from_revert_code(revert_code) {
            ...
            let (error_code, panicking_calls) = if program_abi.spec_version.is_v1_1() {
                ...
                if revert_code & IS_REVERT_CAUSED_BY_PANIC_MASK == 0 {
                    ...
                    let panicking_calls = (0..=4)
                        ...
                        .collect();
                    (Some(error_code), panicking_calls)
                }
            } else {
            ...
            if let Some(error_details) = error_code.and_then(|error_code| {
                program_abi
                    .error_codes
                    .as_ref()
...
mod tests {
    ...
    #[rstest]
    fn new_with_abi_error_code_not_panicking(#[values("1.1", "1.2")] version: &str) {
        let (error_code, revert_code) = if version == "1.1" {
            // In v1.1, all error codes indicate panicking.
            // So, we use a revert code, 112233, that is not the one we will use for the error code.
            (0xffff_ffff_0000_0000_u64, 112233)
        } else if version == "1.2" {
            // In v1.2, we can create a revert code without the panic bit set.
            // This does not indicate panicking. 112233 does not have the panic bit set.
            (0, 112233)
        } else {
            panic!("Unknown version: {version}");
        };
